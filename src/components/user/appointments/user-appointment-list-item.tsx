...
export default function UserAppointmentListItem({ details }: { details: AppointmentDetails }) {
  ...
  return (
    <div className="w-full flex flex-col rounded-2xl overflow-clip bg-[#F9F9F9] inset-shadow-glass shadow-sm">
      {/* SERVICE IMAGE */} 
      ...
      <div className="w-full h-auto flex flex-col gap-3 px-3 py-5">
        ...
        <div className='flex flex-col gap-2'>
          ...
          <div className='flex flex-row items-center gap-1.5'>
            <MapPin width={18} color='#191919' strokeWidth={1.5}/>
            <h2 className='text-[#191919] text-sm'>{appointmentLocation}</h2>  
          </div>
          ...
          <div className='flex flex-row items-center gap-1.5'>
            ...
            <div className='flex flex-row items-center'>
              <h2 className='text-[#191919] text-sm'>{appointmentDate}</h2> 
              <Dot color="#333" strokeWidth={2} className=' w-4'/> 
              <h2 className='text-[#191919]  text-sm'>{appointmentHour}</h2> 
              <Dot color="#333" strokeWidth={2} className='w-4'/> 
              <h2 className='text-[#191919] text-sm'>{appointmentDayOfWeek}</h2>  
            </div>
          </div>
        </div>

        {/* APPOINTMENT STATUS ROW */}
        {details.status == "Zarezerwowana" && <div className='text-xs text-white bg-green-700/90 py-0.5 px-3 rounded-md w-fit'>{status}</div>}
        {details.status == "Odwołana" && <div className='text-xs text-white bg-[#CF142B] py-0.5 px-3 rounded-md w-fit'>{status}</div>}
