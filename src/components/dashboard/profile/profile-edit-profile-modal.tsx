...
export const ProfileEditProfileModal = ({open, onClose, profileData}:ProfileChangePasswordModalProps) => {
    ...
    const {register, handleSubmit, formState : {isSubmitting, errors}, reset} = useForm<ChangeClientProfileData>({
    ...
    const { mutate } = useMutation({
        ...
        mutationFn: async (data: ChangeClientProfileData) => {
            const oldData = {name: profileData.name, email: profileData.email, phone: profileData.phone}
            const result = await updateClientProfileData(oldData, data)
            return result
        }
    })

    // handling changing personal information
    const onSubmit = (data: ChangeClientProfileData) => {
        mutate(data, {
            onSuccess: () => {
    ...
    return createPortal(
        <div className="fixed top-0 left-0 w-full h-lvh z-10">
            {/* OVERLAY  */}
            <div className=" w-full h-full bg-black opacity-65" />
            {/* CONTENT  */}
