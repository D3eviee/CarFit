...
export const  DashboardAppointmentsDetailsModalCancelModal = ({ appointmentId }: {appointmentId: string}) => {
  ...
  const {mutate:deleteAppointmentMutation, isPending:deleteAppointmentIsPending } = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["getAppointmentsTableData"] })
      closeModal()
      closeModal()
    }
  })
