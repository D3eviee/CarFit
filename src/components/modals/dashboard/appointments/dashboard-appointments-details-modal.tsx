...
import { DashboardAppointmentsDetailsModalCancel } from "./dashboard-appointments-details-modal-cancel";

export const DashboardAppointmentsDetailsModal = ({appointmentData}:{appointmentData:CalendarAppointmentOverviewProps}) => {
  ...
  return(
    <div className="w-full h-full flex flex-col bg-white gap-6 md:shadow-2xl md:rounded-3xl md:max-w-[85%] md:max-h-[80%] lg:max-w-3/4 xl:max-w-1/2">
      ...
      {status != "Odwołana" && <div className="w-full py-8">
        <DashboardAppointmentsDetailsModalCancel appointmentId={appointmentId}/>
      </div>}
    </div>
  )
}